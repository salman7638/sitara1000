...
class SaleBookAdvancePaymentInv(models.TransientModel):
    _name = "sale.book.advance.payment.inv"
    _description = "Sales Book Advance Payment Invoice"
    ...
    def default_get(self,  default_fields):
        res = super(SaleBookAdvancePaymentInv, self).default_get(default_fields)
        order_id = self.env['sale.order'].browse(self._context.get('active_ids',[]))
        next_payment_id = self.env['sale.order.payment.line'].search([('order_id','=',order_id.id),('amount_residual','>',0)],order='date_due' ,limit=1)
        res.update({
            'fixed_amount': next_payment_id.amount_installment,
            'advance_payment_method': 'fixed',
            'payment_plan_line_id': next_payment_id.id,
    ...
    booking_order = fields.Boolean('Is Order Booking?', default=False)
    ...
    def _prepare_invoice_values(self, order, name, amount, payment_plan_line_id, so_line):
        invoice_vals = {
            ...
            'payment_plan_line_id': payment_plan_line_id,
    ...
    def _create_invoice(self, order, so_line, amount,payment_plan_line_id):
        ...
        invoice_vals = self._prepare_invoice_values(order, name, amount, payment_plan_line_id, so_line)
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            ...
            for order in sale_orders:
                ...
                self._create_invoice(order, so_line, amount,payment_plan_line_id)
                order.action_confirm()
                if self.booking_order:
                    order.action_property_reserve()
